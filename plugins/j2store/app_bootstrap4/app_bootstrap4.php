...
class plgJ2StoreApp_bootstrap4 extends J2StoreAppPlugin
{
    ...
    function onJ2StoreTemplateFolderList(&$folder){
        if(!in_array('bootstrap4',$folder)){
            $folder[] = 'bootstrap4';
        }
        if(!in_array('tag_bootstrap4',$folder)){
            $folder[] = 'tag_bootstrap4';
        }
    }
    ...
    function setTemplatePath($view,$default = 'bootstrap4'){
        $app = J2Store::platform()->application();
    ...
    function onJ2StoreViewProductListTagHtml(&$view_html, &$view, $model){
        F0FPlatform::getInstance()->setErrorHandling(E_ALL, 'ignore');
        $view = $this->setTemplatePath($view,'tag_bootstrap4');
        $result = $view->loadTemplate();
