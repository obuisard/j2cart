...
class plgJ2StoreApp_bootstrap3 extends J2StoreAppPlugin
{
    /**
     * @var $_element  string  Should always correspond with the plugin's filename,
     *                         forcing it to be unique
     */
    var $_element   = 'app_bootstrap3';
    ...
    function onJ2StoreTemplateFolderList(&$folder){
        if(!in_array('bootstrap3',$folder)){
            $folder[] = 'bootstrap3';
        }
        if(!in_array('tag_bootstrap3',$folder)){
            $folder[] = 'tag_bootstrap3';
        }
    }
    ...
    function setTemplatePath($view,$default = 'bootstrap3'){
        $app = J2Store::platform()->application();
    ...
    function onJ2StoreViewProductListTagHtml(&$view_html, &$view, $model){
        F0FPlatform::getInstance()->setErrorHandling(E_ALL, 'ignore');
        $view = $this->setTemplatePath($view,'tag_bootstrap3');
        $result = $view->loadTemplate();
