<?php
/**
 * @package     Joomla.Component
 * @subpackage  J2Store
 *
 * @copyright Copyright (C) 2014-24 Ramesh Elamathi / J2Store.org
 * @copyright Copyright (C) 2025 J2Commerce, LLC. All rights reserved.
 * @license https://www.gnu.org/licenses/gpl-3.0.html GNU/GPLv3 or later
 * @website https://www.j2commerce.com
 */

defined('_JEXEC') or die;

use Joomla\CMS\Form\Field\ListField;
use Joomla\CMS\Language\Text;

require_once JPATH_ADMINISTRATOR.'/components/com_j2store/helpers/j2html.php';

class JFormFieldTaxSelect extends ListField
{
	protected $type = 'Taxselect';

	public function getInput()
    {
		$model = J2Store::fof()->getModel('Taxprofiles','J2StoreModel');
		$list = $model->enabled(1)->getItemList();
		$attr = [];
		// Get the field options.
		// Initialize some field attributes.
        $attr['class'] = !empty($this->class) ? (strpos($this->class, 'form-select') === false ? $this->class . ' form-select' : $this->class) : 'form-select';

		$attr ['onchange']= $this->onchange ?  $this->onchange : '';
		//generate tax filter list
		$options = [];
		$options[''] =  Text::_('J2STORE_NOT_TAXABLE');
		foreach($list as $row) {
			$options[$row->j2store_taxprofile_id] =  Text::_($row->taxprofile_name);
		}
		return J2Html::select()->clearState()
						->type('genericlist')
						->name($this->name)
						->attribs($attr)
						->value($this->value)
						->setPlaceHolders($options)
						->getHtml();
	}
}
